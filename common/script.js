...
if ('IntersectionObserver' in window) { // ブラウザが対応してるかチェック
    // IntersectionObserver…「要素が画面にどれくらい入ってきたか」を教えてくれる便利API
    const observer = new IntersectionObserver((entries) => {
    // entries: 監視中の要素に対して「入ってきた/出ていった」がまとまって渡ってくる
        entries.forEach((entry) => {
            if (entry.isIntersecting) entry.target.classList.add('sectionFadeIn');
            // isIntersecting=true…対象がビューポートに入ってきた合図
            // .show を付与 → CSS側で opacity/transform を元に戻す（＝ふわっと表示）
        });
    }, { threshold: 0.1 }); // threshold: 20%入ってきたら反応する（0〜1の割合）

    sections.forEach((sec) => observer.observe(sec)); // すべての<section>を監視開始
} else {
    // 古いブラウザ（Observer非対応）のときは、とりあえず全部見せる処理にフォールバック
